...
Based on the provided requirements, the Blitzy platform understands that the testing objective is to **create comprehensive unit tests for the Express.js server application (server.js) using either Jest or Mocha as the testing framework**. The tests must provide thorough coverage of HTTP server functionality, ensuring the application behaves correctly under various scenarios.

**Testing Request Categorization:** Add new tests (greenfield testing for an untested codebase)

**Specific Testing Requirements:**

- **HTTP Response Testing:** Validate that HTTP endpoints return correct response bodies with expected content
- **Status Code Verification:** Ensure appropriate HTTP status codes are returned for successful requests (200) and undefined routes (404)
- **Header Validation:** Verify that response headers are set correctly, including Content-Type and Content-Length
- **Server Lifecycle Testing:** Test server startup process, port binding, and graceful shutdown capabilities
- **Error Handling Coverage:** Validate error scenarios including invalid routes, server errors, and edge cases
- **Edge Case Testing:** Test boundary conditions, malformed requests, and unusual input scenarios

**Implicit Testing Needs Identified:**

- **Request Method Testing:** Verify correct handling of different HTTP methods (GET, POST, PUT, DELETE) even though current implementation only has GET routes
- **Port Binding Verification:** Ensure server binds to the correct hostname (127.0.0.1) and port (3000)
- **Middleware Integration:** Test that Express middleware stack processes requests correctly
- **Asynchronous Operation Testing:** Validate that server handles async operations and callbacks properly
- **Memory Leak Prevention:** Ensure proper cleanup after server shutdown to prevent resource leaks
- **Concurrent Request Handling:** Test that the server can handle multiple simultaneous requests

### 0.1.2 Special Instructions and Constraints

**Critical Directives:**

- **Testing Framework Choice:** Implement tests using Jest as the primary framework due to its superior developer experience, built-in assertion library, comprehensive mocking capabilities, and excellent async/await support
- **Minimal Code Changes:** DO NOT modify server.js unless absolutely necessary for testability - focus only on creating test files
- **Repository Test Conventions:** Follow Node.js and Express.js testing best practices since no existing test patterns exist in this repository
- **Non-Interactive Execution:** All test commands must run in CI/CD environments without requiring user input

**User-Provided Examples:** None provided

**Web Search Requirements Identified:**

- Best practices for testing Express.js 5.x applications with Jest
- Recommended patterns for testing HTTP servers with supertest library
- Coverage thresholds and quality metrics for Node.js backend applications
- Testing strategies for server startup and shutdown lifecycle
- Error handling test patterns for Express middleware

### 0.1.3 Technical Interpretation

These testing requirements translate to the following technical test implementation strategy:

**To test HTTP responses and endpoints**, we will create comprehensive test suites in `tests/server.test.js` that use supertest to make actual HTTP requests to the Express application without needing to start a real server.

**To verify status codes**, we will implement assertions using Jest's `expect()` API combined with supertest's fluent interface to validate response status codes for both successful (200) and error (404) scenarios.

**To validate response headers**, we will use supertest's header assertion methods to verify Content-Type, Content-Length, and other relevant HTTP headers are set correctly by Express.

**To test server lifecycle**, we will create separate test suites that initialize the Express app, verify it listens on the correct port, and implement proper teardown procedures to close server connections after tests complete.

**To cover error handling**, we will create test cases that trigger error conditions including 404 errors for undefined routes, malformed requests, and verify that Express error middleware handles exceptions gracefully.

**To address edge cases**, we will implement boundary condition tests including empty paths, special characters in URLs, very long URLs, rapid successive requests, and server behavior under various load conditions.

### 0.1.4 Coverage Requirements Interpretation

**Explicit Coverage Targets:** User requirement indicates "comprehensive unit tests" which translates to thorough testing of all functionality in server.js.

**Implicit Coverage Expectations:**

Based on industry standards for Node.js/Express applications:
- **Line Coverage:** Minimum 85% line coverage for all executable code in server.js
- **Branch Coverage:** Minimum 80% branch coverage for conditional logic
- **Function Coverage:** 100% function coverage - all exported functions must have tests
- **Statement Coverage:** Minimum 85% statement coverage

Based on existing patterns in the repository:
- No existing test coverage patterns to follow - this is greenfield testing
- Repository emphasizes simplicity and educational value, suggesting tests should be clear and well-documented

Based on critical path analysis:
- **Root endpoint (/):** Must have multiple test cases covering happy path, response format, and headers
- **Evening endpoint (/evening):** Must have complete test coverage for response validation
- **404 handling:** Must verify undefined routes return appropriate error responses
- **Server initialization:** Must test successful startup and configuration
- **Server shutdown:** Must test graceful cleanup and connection termination

**Comprehensive Coverage Goal Statement:**

To achieve comprehensive testing, coverage should include:
- All HTTP endpoints (/ and /evening) with multiple test scenarios each
- Server startup and shutdown lifecycle events
- Request handling for various HTTP methods
- Response body content validation with exact string matching
- Response header verification including Content-Type and Content-Length
- Status code validation for both success (200) and error (404) cases
- Error boundary testing for malformed requests and server errors
...
A comprehensive search of the repository revealed the following findings:

**Search Patterns Employed:**
- Searched for test files matching patterns: `*test*`, `*spec*`, `test_*`, `spec_*`, `*_test.*`, `*_spec.*`
- Searched for test directories: `test/`, `tests/`, `__tests__/`, `spec/`, `specs/`
- Examined package.json and package-lock.json for testing framework dependencies
- Searched for test configuration files: `jest.config.*`, `.mocharc.*`, `vitest.config.*`, `karma.conf.*`
- Checked for CI/CD configuration files: `.travis.yml`, `.circleci/`, `azure-pipelines.yml`, `.github/workflows/`, `Jenkinsfile`, `.gitlab-ci.yml`

**Repository Analysis Findings:**

Repository analysis reveals **NO existing testing infrastructure**. The repository contains:
- **No test files:** Zero test files found in the codebase
- **No test directories:** No dedicated test folders exist
- **No testing frameworks:** package.json contains only `express: ^5.1.0` as a dependency with no testing libraries
- **No test configuration:** No jest.config.js, .mocharc.js, or other test configuration files
- **No CI/CD setup:** No continuous integration configuration files detected
- **No coverage tools:** No coverage reporting tools configured

This is a **greenfield testing scenario** - we will be building the entire test infrastructure from scratch.

**Current Testing Framework:** None - framework selection required

**Test Runner Configuration Location:** None exists - will be created at `jest.config.js`

**Coverage Tools in Use:** None - will implement Jest's built-in coverage reporter

**Mock/Stub Libraries Detected:** None - will use Jest's built-in mocking capabilities

**Test Data Fixtures or Factories Present:** None - test data will be inline or in separate fixture files if needed

### 0.2.2 Web Search Research Conducted

**Best Practices Research:**

- **Jest with Node.js 18 and Express 5 Compatibility:** <cite index="9-1">Jest 29+ supports Node.js versions 14.15, 16.10, 18.0 and above</cite>, confirming full compatibility with the project's Node.js v18.20.8 requirement. <cite index="14-1">Jest 30.2.0 is the latest stable version</cite>.

- **Express 5 Testing Considerations:** <cite index="2-1,3-1">Express.js 5 requires Node.js 18 or higher</cite>, which aligns perfectly with our environment setup.

- **Supertest for HTTP Testing:** <cite index="31-1,31-2">Supertest is a SuperAgent driven library for testing HTTP servers, with version 7.1.4 as the latest</cite>. It integrates seamlessly with both Jest and Express without requiring the server to listen on an actual port.

- **Mocha Alternative:** <cite index="22-2">Mocha version 11.7.4 is available</cite>, but Jest is recommended for this project due to its zero-configuration philosophy, built-in assertions, and superior async/await support.

**Testing Pattern Recommendations:**

The research indicates that modern Express testing follows these patterns:
- Export the Express app instance without calling `app.listen()` to enable testing without port conflicts
- Use supertest to make HTTP requests directly to the app instance
- Leverage Jest's async/await support for cleaner test code
- Implement proper setup and teardown hooks using `beforeAll()`, `afterAll()`, `beforeEach()`, and `afterEach()`
- Use Jest's built-in matchers for assertions rather than third-party libraries
- Configure Jest's testEnvironment to "node" for server-side testing

**Common Pitfalls to Avoid:**

- Not properly closing server connections in test teardown, leading to hanging processes
- Testing against a running server on a fixed port, causing port conflicts
- Not handling async operations correctly, causing flaky tests
- Over-mocking when integration testing would be more valuable
- Not testing error conditions and edge cases thoroughly
...
- **Module:** server.js (18 lines, Express HTTP server)
- **Location:** `/server.js` (project root)
- **Test Type Required:** Integration tests (testing HTTP endpoints and server lifecycle)

**Functions and Endpoints Requiring Tests:**

**Root Endpoint (`GET /`):**
- Test Categories: Response body validation, status code verification, header validation, content-type checking
- Expected Behavior: Returns "Hello, World!\n" with status 200

**Evening Endpoint (`GET /evening`):**
- Test Categories: Response body validation, status code verification, header validation  
- Expected Behavior: Returns "Good evening" with status 200

**404 Error Handling:**
- Test Categories: Error response validation, status code verification
- Expected Behavior: Undefined routes return 404 status

**Server Lifecycle:**
- Test Categories: Startup validation, port binding, graceful shutdown
- Expected Behavior: Server listens on 127.0.0.1:3000

**Existing Test File Mapping:**

| Source File | Existing Test File | Test Categories Present |
|-------------|-------------------|------------------------|
| server.js | None | No existing tests |

**Dependencies Requiring Mocking:**

No external dependencies require mocking for the current server.js implementation:
- **Express Framework:** Will be imported directly - no mocking needed as we're integration testing
- **HTTP Module:** Implicitly used by Express - will be tested through supertest
- **No External Services:** The server has no database, API calls, or external service dependencies
- **No File System Operations:** No file I/O to virtualize
- **No Environment Dependencies:** Server uses hardcoded values, no environment variable dependencies to mock

**Testing Strategy:** Pure integration testing without mocks, using supertest to simulate HTTP requests directly to the Express app instance.

### 0.3.2 Version Compatibility Research

**Based on Node.js v18.20.8 and Express 5.1.0, the recommended testing stack is:**

**Testing Framework: Jest version 30.2.0**
- **Compatibility Rationale:** <cite index="9-1">Jest 29+ supports Node.js 18.0 and above</cite>, and <cite index="14-1">Jest 30.2.0 is the latest stable release</cite>, providing full compatibility with Node.js 18.20.8
- **Selection Justification:** Jest offers zero-configuration setup, built-in assertion library, excellent async/await support, built-in coverage reporting, and snapshot testing capabilities
- **Breaking Changes:** Jest 30 requires Node.js 18+ minimum, which matches our environment

**HTTP Testing Library: supertest version 7.1.4**
- **Compatibility Rationale:** <cite index="31-2">Supertest 7.1.4 is the latest version</cite> and works with any Node.js HTTP server including Express 5
- **Selection Justification:** Supertest is specifically designed for testing HTTP servers, integrates seamlessly with Jest, requires no actual server startup, and provides fluent assertion API
- **Express 5 Compatibility:** <cite index="1-1,3-1">Express 5 requires Node.js 18 or higher</cite>, ensuring full compatibility with our setup

**Assertion Library: Jest's built-in matchers (included with Jest 30.2.0)**
- **Compatibility Rationale:** Native Jest matchers work out of the box with no additional dependencies
- **Selection Justification:** Provides comprehensive assertion capabilities including `toBe()`, `toEqual()`, `toMatch()`, `toContain()`, and async matchers

**Coverage Tool: Jest's built-in coverage reporter (included with Jest 30.2.0)**
- **Compatibility Rationale:** Jest includes Istanbul coverage reporting by default
- **Selection Justification:** Zero additional configuration required, supports multiple output formats (text, lcov, html), integrated into jest command with `--coverage` flag

**Version Conflicts to Resolve:** None identified - all selected tools are mutually compatible and work with Node.js 18.20.8 and Express 5.1.0
...
- **Scope:** Testing HTTP endpoints end-to-end through the Express application
- **Coverage:** All routes (/, /evening), 404 handling, and server lifecycle
- **Rationale:** The server.js file is a complete HTTP server application best tested through actual HTTP requests rather than isolated unit tests
- **Implementation:** Use supertest to make HTTP requests without starting an actual server

**Server Lifecycle Tests:**
- **Scope:** Testing server initialization, port binding, and graceful shutdown
- **Coverage:** Server startup verification, connection handling, cleanup procedures
- **Rationale:** Ensures the server starts correctly and cleans up resources properly

**Edge Case Tests:**
- **Scope:** Boundary conditions and unusual request scenarios
- **Coverage:** Invalid routes, malformed requests, special characters, edge-case HTTP methods
- **Rationale:** Validates robust error handling and prevents unexpected failures

**Error Handling Tests:**
- **Scope:** Testing how the application handles error conditions
- **Coverage:** 404 errors, undefined routes, server error scenarios
- **Rationale:** Ensures graceful degradation and appropriate error responses

### 0.4.2 Test Case Blueprint

**Component: Root Endpoint (`GET /`)**

Test Categories:
- **Happy path:** 
  - Request to "/" returns status 200
  - Response body contains "Hello, World!\n"
  - Content-Type header is set to "text/html; charset=utf-8"
  - Response completes successfully without errors
  
- **Edge cases:** 
  - Request with query parameters (/?test=value)
  - Request with trailing slash (//)
  - Request with accept headers
  
- **Performance boundaries:** 
  - Response time under normal conditions
  - Server handles concurrent requests to root endpoint

**Component: Evening Endpoint (`GET /evening`)**

Test Categories:
- **Happy path:** 
  - Request to "/evening" returns status 200
  - Response body equals "Good evening"
  - Content-Type header is correct
  - Content-Length header matches response size
  
- **Edge cases:** 
  - Request to "/evening/" with trailing slash
  - Request with various HTTP methods (POST, PUT, DELETE should work per Express 5 default behavior)

**Component: 404 Error Handling**

Test Categories:
- **Error cases:** 
  - Undefined route returns 404 status
  - GET /nonexistent returns 404
  - Invalid paths return appropriate error response
  - Error response format is HTML (default Express error handler)
...
- **Initialization:** 
  ...
  - Server binds to correct host (127.0.0.1) and port (3000)
...
### 0.4.3 Test Data and Fixtures Design

**Required Test Data Structures:** None - server has no database or complex data structures

**Fixture Organization Strategy:** Inline test data within test files for simplicity

**Mock Object Specifications:** No mocks required - integration testing approach

**Test Database/State Management Approach:** 
- **No State:** Server is stateless - no database or persistent state
- **Cleanup Strategy:** Close server connections in `afterAll()` hook
- **Isolation:** Each test runs independently using supertest without shared state

## 0.5 Test File Transformation Mapping

### 0.5.1 File-by-File Test Plan

**Comprehensive Test File Transformation Map:**

| Target Test File | Transformation | Source File/Test | Purpose/Changes |
|-----------------|----------------|------------------|-----------------|
| tests/server.test.js | CREATE | server.js | Create comprehensive integration tests for all HTTP endpoints (/, /evening), 404 error handling, server lifecycle (startup/shutdown), response validation (status codes, headers, body content), and edge cases |
| tests/server.lifecycle.test.js | CREATE | server.js | Create dedicated tests for server initialization, port binding verification, graceful shutdown, connection handling, and resource cleanup to ensure proper lifecycle management |
| jest.config.js | CREATE | N/A | Create Jest configuration file with testEnvironment set to "node", coverage thresholds (85% line, 80% branch), collectCoverageFrom patterns, and test path patterns |
| .gitignore | UPDATE | .gitignore | Add Jest-specific entries: coverage/, *.log, .jest-cache/ to prevent committing test artifacts |
| package.json | UPDATE | package.json | Add test script ("test": "jest"), test:watch script, test:coverage script, and devDependencies for jest@30.2.0 and supertest@7.1.4 |
| README.md | UPDATE | README.md | Add testing section documenting how to run tests, coverage reports, test structure, and testing best practices |

**File Analysis:** ALL test files expected to be in scope have been identified above. No files left as "pending" or "to be discovered."

### 0.5.2 New Test Files Detail

**tests/server.test.js** - Main integration test suite for HTTP endpoints

Test Categories:
- **Happy path tests:** Verify / and /evening endpoints return correct responses with proper status codes
- **Edge cases:** Test query parameters, trailing slashes, special characters in URLs
- **Error scenarios:** Validate 404 responses for undefined routes

Mock Dependencies: None required

Assertions Focus:
- HTTP status code validation using `expect(response.status).toBe(200)`
- Response body content matching using `expect(response.text).toBe("Hello, World!\\n")`
- Header verification using `expect(response.headers['content-type']).toMatch(/text\/html/)`
- Response timing and performance characteristics

**tests/server.lifecycle.test.js** - Server lifecycle management tests

Test Categories:
- **Startup tests:** Verify server initializes correctly and binds to 127.0.0.1:3000
- **Shutdown tests:** Ensure graceful cleanup of connections and resources
- **Concurrent request handling:** Test multiple simultaneous requests

Mock Dependencies: None required

Assertions Focus:
- Server listening state verification
...
**jest.config.js** - Create new Jest configuration file with:
- `testEnvironment: 'node'` - Configure Node.js test environment instead of browser/jsdom
- `coverageThreshold: { global: { branches: 80, functions: 100, lines: 85, statements: 85 } }` - Set coverage requirements
- `collectCoverageFrom: ['server.js']` - Specify which files to include in coverage reports
- `testMatch: ['**/tests/**/*.test.js']` - Define test file patterns
- `coverageDirectory: 'coverage'` - Set output directory for coverage reports
- `verbose: true` - Enable detailed test output

**package.json** - Update scripts section:
- Add `"test": "jest"` - Run all tests once
- Add `"test:watch": "jest --watch"` - Run tests in watch mode for development
- Add `"test:coverage": "jest --coverage"` - Run tests with coverage report
- Add `"test:verbose": "jest --verbose"` - Run tests with detailed output

**package.json** - Update devDependencies:
- Add `"jest": "^30.2.0"` - Testing framework
- Add `"supertest": "^7.1.4"` - HTTP integration testing library

**.gitignore** - Add test artifacts to ignore:
...
- Coverage requirements and how to generate reports
- Test structure and organization
- Best practices for writing new tests

### 0.5.4 Cross-File Test Dependencies

**Shared Fixtures:** None required - tests use inline test data

**Mock Objects:** None required - integration testing approach without mocks

**Test Utilities:** None required initially - Jest and supertest provide all necessary utilities

**Import Updates Required:** No import changes needed in test files - they will import:
- `const request = require('supertest')` - For HTTP testing
- `const app = require('../server')` - Will require exporting app from server.js (see note below)

**IMPORTANT TESTABILITY MODIFICATION:**

To enable testing, server.js requires ONE minimal modification:
- **Current:** Server creates app and immediately calls `app.listen()`
- **Required:** Separate app creation from server startup by exporting the app
- **Change:** Export `app` before calling `app.listen()`, and only start server if run directly (not required)
- **Impact:** Allows tests to import app without starting server, enabling supertest integration

## 0.6 Dependency Inventory

### 0.6.1 Testing Dependencies

**Key Testing Packages for This Testing Exercise:**

| Registry | Package Name | Version | Purpose |
|----------|--------------|---------|---------|
| npm | jest | 30.2.0 | Primary testing framework providing test runner, assertion library, mocking capabilities, and coverage reporting |
| npm | supertest | 7.1.4 | HTTP integration testing library for making requests to Express app and asserting responses without starting actual server |

**Dependency Notes:**

- **jest@30.2.0** is the latest stable version providing comprehensive testing capabilities including built-in assertion matchers, async/await support, parallel test execution, snapshot testing, and Istanbul code coverage reporting
- **supertest@7.1.4** is the latest version specifically designed for testing HTTP servers, integrating seamlessly with Express and any test framework
- **No additional assertion libraries needed** - Jest's built-in matchers (`expect`, `toBe`, `toEqual`, `toMatch`, etc.) provide all necessary assertion capabilities
- **No mocking libraries needed** - Jest includes powerful mocking functionality built-in
- **No test reporters needed** - Jest includes multiple reporters (default, verbose, JSON, lcov for coverage)
...
| Registry | Package Name | Version | Purpose |
|----------|--------------|---------|---------|
| npm | express | 5.1.0 | Web framework for creating HTTP server (already installed) |

**No Changes to Production Dependencies:** Testing will be added as devDependencies only, keeping production dependencies unchanged.
...
- **tests/server.test.js** - Requires:
  ...
  - `const app = require('../server');` - Import Express app instance from server.js
  
- **tests/server.lifecycle.test.js** - Requires:
  - `const request = require('supertest');` - Import supertest for HTTP testing
  - `const app = require('../server');` - Import Express app instance from server.js

**Source File Modification (Minimal Testability Change):**

- **server.js** - Requires:
  - `module.exports = app;` - Export Express app before starting server
  - Conditional server startup (optional but recommended): 
    ```javascript
    ...
    ```
  - **Rationale:** Allows tests to import app without automatically starting server, preventing port conflicts and enabling supertest integration testing

**No Import Transformation Rules Needed:** Tests will use standard CommonJS `require()` statements matching the existing server.js module pattern.
...
Based on user requirement for "comprehensive unit tests" and industry best practices for backend Node.js applications:

- **Line Coverage Target:** 85%
  - **Rationale:** Standard industry benchmark for backend services ensures most code paths are executed during tests
  - **Measurement:** Percentage of executable lines of code that are executed by test suite
  
- **Branch Coverage Target:** 80%
  - **Rationale:** Ensures conditional logic and control flow paths are tested
  - **Measurement:** Percentage of decision branches (if/else, switch cases) executed by tests
  
- **Function Coverage Target:** 100%
  - **Rationale:** All exported functions and endpoint handlers should have at least one test
  - **Measurement:** Percentage of functions called during test execution
  
- **Statement Coverage Target:** 85%
  - **Rationale:** Similar to line coverage, ensures comprehensive statement execution
  - **Measurement:** Percentage of code statements executed by tests

**Coverage Gaps to Address:**

Currently, the following areas have 0% coverage and require tests:

| Component | Current Coverage | Target Coverage | Focus Areas |
|-----------|-----------------|-----------------|-------------|
| Root endpoint (/) | 0% | 100% | Response body validation, status code 200, headers, edge cases |
| Evening endpoint (/evening) | 0% | 100% | Response body validation, status code 200, headers |
| 404 Error handling | 0% | 100% | Undefined route handling, proper error responses |
| Server initialization | 0% | 100% | App creation, middleware setup, port binding |
| Request/response cycle | 0% | 100% | Complete HTTP request handling flow |

**Per-File Coverage Targets:**

- **server.js:** 85% minimum coverage across all metrics
  - All routes must be tested (100% route coverage)
  - Error handling paths must be covered (80% branch coverage)
  - Server lifecycle must be validated

### 0.7.2 Test Quality Criteria

**Assertion Density Expectations:**

- **Minimum 3 assertions per test case:** Each test should verify status code, response body, and at least one header
- **Comprehensive endpoint validation:** Tests must check response.status, response.text/body, and response.headers
- **Error case assertions:** Error tests must validate both status code and error response format

**Test Isolation Requirements:**

- **No shared state between tests:** Each test must be independent and not rely on execution order
- **Supertest isolation:** Use supertest's app instance approach to ensure tests don't interfere with each other
- **No global variables:** Avoid globals that could cause test interdependencies
- **Proper cleanup:** Use `afterAll()` and `afterEach()` hooks to clean up resources

**Performance Constraints for Test Execution:**

- **Total test suite runtime:** Should complete in under 5 seconds for current scope
- **Individual test timeout:** No single test should exceed 5000ms (Jest default timeout)
- **Startup/shutdown overhead:** Server lifecycle tests should complete quickly without actual network delays
- **Parallel execution ready:** Tests should be designed to run in parallel (Jest default behavior)

**Maintainability Standards:**

- **Clear test descriptions:** Use descriptive test names that explain what is being tested and expected outcome
- **DRY principle:** Extract common setup code into `beforeEach()` and `beforeAll()` hooks
- **Readable assertions:** Use Jest's semantic matchers (toBe, toEqual, toMatch) for clarity
- **Documentation:** Include comments for complex test scenarios or edge cases
- **Consistent structure:** Follow AAA pattern (Arrange, Act, Assert) in all tests

**Following Repository Test Patterns and Conventions:**

Since no existing test patterns exist in this repository:
- **Establish conventions:** These tests will set the standard for future testing in the project
- **Node.js best practices:** Follow community-accepted Node.js testing patterns
- **Express testing conventions:** Use supertest as the standard approach for HTTP endpoint testing
- **Jest conventions:** Leverage Jest's best practices including describe/it structure, beforeEach/afterAll hooks, and built-in matchers
...
- `tests/server.test.js` - Main integration test suite for HTTP endpoints
- `tests/server.lifecycle.test.js` - Server initialization and shutdown tests
- `tests/**/*.test.js` - All Jest test files following this pattern

**Test Configuration Files:**
- `jest.config.js` - Jest test framework configuration
- `.jestrc.json` - Alternative Jest configuration format (if used)
- `package.json` - Test scripts and devDependencies sections

**Test Utilities and Helpers:**
- None required initially - may be added later if test complexity increases
- `tests/helpers/**/*.js` - Potential location for future test utilities
- `tests/fixtures/**/*.js` - Potential location for test data if needed

**Documentation Updates:**
- `README.md` - Testing section with instructions on running tests and interpreting results
- `README.md` - Coverage requirements and quality standards documentation

**Source Code Modifications (Minimal, for Testability Only):**
- `server.js` - Add `module.exports = app;` to export Express app for testing
- `server.js` - Optionally wrap `app.listen()` in conditional to prevent automatic startup when imported

**Version Control Configuration:**
- `.gitignore` - Add Jest-specific patterns: `coverage/`, `.jest-cache/`, test log files

### 0.8.2 Explicitly Out of Scope

**Source Code Modifications Beyond Testability:**
- Refactoring server.js logic or restructuring code
- Adding new features or functionality to the Express server
- Changing existing endpoint behavior or responses
- Adding middleware or modifying the Express app configuration
- Performance optimizations unrelated to test coverage
- Security enhancements not directly related to testing

**Non-Testing Dependencies:**
- Adding production dependencies beyond express
- Adding linting tools (ESLint, Prettier) - separate concern
- Adding build tools or transpilers
- Adding deployment configuration

**Infrastructure and CI/CD:**
- Setting up GitHub Actions or other CI/CD pipelines (may be future work)
- Docker configuration for test environments
- Cloud deployment configurations
- Database setup or ORM configuration (not applicable to this stateless server)

**Unrelated Test Categories:**
- End-to-end (E2E) browser tests - server only, no frontend
- Load testing or stress testing - beyond unit/integration test scope
- Security penetration testing - separate security audit concern
- Performance benchmarking - not requested in requirements

**Files Explicitly Excluded:**
- `blitzy/**/*` - Documentation folder not relevant to testing
- `node_modules/**/*` - Third-party dependencies
- `coverage/**/*` - Generated test artifacts
- `.git/**/*` - Version control metadata

**Scope Limitations Per User Instructions:**
- User specified testing server.js only - no other source files in scope
- User requested comprehensive unit tests - not load tests, security tests, or E2E tests
- Focus is on HTTP responses, status codes, headers, error handling, and server lifecycle only
...
npm test
```
Executes all test files matching `tests/**/*.test.js` pattern using Jest test runner.

**Run Tests with Coverage Report:**
```bash
npm run test:coverage
```
Executes all tests and generates comprehensive coverage report in `coverage/` directory with lcov and HTML formats.

**Run Tests in Watch Mode (Development):**
```bash
npm run test:watch
```
Runs Jest in watch mode, automatically re-running tests when source files change. Useful during test development.

**Run Tests with Verbose Output:**
```bash
npm run test:verbose
```
Executes tests with detailed output showing individual test results and timing information.

**Run Specific Test File:**
```bash
npx jest tests/server.test.js
```
Executes only the specified test file, useful for focused testing during development.

**Run Tests Matching Pattern:**
```bash
npx jest --testNamePattern="GET /"
```
Runs only tests whose description matches the specified pattern.

**Coverage Measurement Command:**
```bash
npx jest --coverage --collectCoverageFrom="server.js"
```
Generates coverage report specifically for server.js with detailed metrics.

**Single Test Execution Pattern:**
```bash
npx jest tests/server.test.js -t "should return Hello World"
```
Executes a single test case matching the specified name within a test file.

**Debug Mode Execution:**
```bash
node --inspect-brk node_modules/.bin/jest --runInBand tests/server.test.js
```
Runs tests in debug mode with Chrome DevTools integration, executing tests serially for easier debugging.

**CI/CD Non-Interactive Execution:**
```bash
CI=true npm test -- --coverage --maxWorkers=2
```
Runs tests in CI environment with coverage, limited workers, and non-interactive mode.

### 0.9.2 Test Development Workflow

**Environment Setup for Testing:**
- Ensure Node.js v18.20.8 is active: `node --version`
- Install test dependencies: `npm install --save-dev jest@30.2.0 supertest@7.1.4`
- Verify installation: `npx jest --version`

**Specific Test Patterns to Follow:**

- **Test File Naming:** Use `.test.js` suffix for all test files (e.g., `server.test.js`)
- **Test Suite Organization:** Group related tests using `describe()` blocks
- **Test Case Structure:** Use `it()` or `test()` for individual test cases
- **Async Testing:** Always use `async/await` pattern for supertest requests
- **Setup/Teardown:** Use `beforeAll()`, `afterAll()`, `beforeEach()`, `afterEach()` hooks appropriately
...
**No Environment Variables Required:** The server uses hardcoded host and port values, so no environment configuration is needed for testing.

**Test Execution Environment:**
- **Node.js Version:** v18.20.8 (exact version used in development)
- **Test Environment:** Node.js (not browser/jsdom) - configured in jest.config.js
- **Timezone:** Not applicable - no time-dependent logic
- **Network:** No external network access required - tests run in isolation

**Excluded Test Categories:**
- Integration tests requiring external APIs - not applicable to this stateless server
- Database integration tests - no database in this application
- Browser-based tests - server-only application
- Performance/load tests - beyond unit/integration test scope
...
- **CRITICAL:** ONLY modify test files, test-related configurations, and documentation
- **DO NOT** modify server.js business logic, endpoint behavior, or response content
- **EXCEPTION:** Minimal testability modification is permitted - adding `module.exports = app;` to server.js to enable testing
- **RATIONALE:** Tests should validate existing behavior without altering it

**Source Code Modification Constraints:**
- **DO NOT modify** endpoint routes or response content
- **DO NOT add** new middleware or Express configuration
- **DO NOT refactor** existing server.js code structure
- **DO modify** ONLY to export the Express app instance for testing purposes
- **OPTIONAL:** Wrap `app.listen()` in conditional check to prevent automatic startup when imported

**Test Pattern Adherence:**
- **Follow Express testing best practices** as documented in Express.js and supertest official documentation
- **Use supertest's fluent API** for making HTTP assertions rather than manual HTTP clients
- **Maintain test isolation** by using supertest's app instance approach rather than starting actual servers
- **Follow Jest conventions** including describe/it structure, beforeEach/afterAll hooks, and semantic matchers

**Mocking and Testing Strategy:**
- **DO NOT mock Express framework** - test the actual Express application
- **DO NOT mock HTTP module** - use supertest for real HTTP request simulation
- **Use integration testing approach** - test endpoints end-to-end through the full Express stack
- **NO external mocking libraries needed** - Jest's built-in mocking is sufficient if needed

**Test Execution Requirements:**
- **Ensure all tests can run independently** - no test should depend on another test's execution or state
- **Ensure all tests can run in parallel** - Jest's default parallel execution should work without issues
- **Ensure tests are deterministic** - tests should produce same results on every run
- **Ensure proper cleanup** - close all connections and release resources in afterAll/afterEach hooks

**Code Style and Naming Conventions:**
- **Match existing code style** in server.js (CommonJS modules, ES5 syntax)
- **Use descriptive test names** that clearly state what is being tested and expected outcome
- **Use consistent assertion style** with Jest's `expect().toBe()` pattern
- **Follow naming convention:** `tests/{moduleName}.test.js` for test files

### 0.10.2 Quality and Maintainability Standards

**Backward Compatibility:**
- **DO NOT change** existing server.js behavior or API contracts
- **Maintain compatibility** with Node.js v18.20.8 and Express 5.1.0
- **Preserve** current server response formats and status codes
- **Ensure** tests document and enforce existing behavior

**Test Documentation Requirements:**
- **Add README section** explaining how to run tests and interpret results
- **Document coverage thresholds** and why they were chosen
- **Explain test structure** and organization for future maintainers
- **Provide examples** of how to add new tests following established patterns

**Error Handling in Tests:**
- **Use try/catch blocks** sparingly - prefer Jest's async error handling
- **Test error scenarios explicitly** - create tests that expect errors and validate error responses
- **Fail fast** - tests should fail immediately on first assertion failure for clear feedback
- **Provide meaningful error messages** using custom messages in assertions when helpful

### 0.10.3 Express 5 Specific Considerations

**Express 5 Compatibility Notes:**
- <cite index="3-1">Express 5 requires Node.js 18 or higher</cite> - already satisfied by v18.20.8
- <cite index="3-3,3-14">Express 5 includes promise support for middleware</cite> - tests can rely on promise-based async handling
- <cite index="1-18">Express 5 uses path-to-regexp@8.x with ReDoS mitigation</cite> - tests should validate route matching behavior
- Default error handlers in Express 5 may differ from Express 4 - validate actual error response formats

**Testing Best Practices for Express 5:**
- Test that middleware can return rejected promises and they're caught by the router
- Verify that status codes are valid - Express 5 throws errors for invalid status codes
- Test actual server behavior rather than assuming Express 4 patterns
- Use supertest to test the complete Express 5 request/response cycle

### 0.10.4 Critical Success Criteria

**Tests Must:**
- ✓ Achieve minimum 85% line coverage for server.js
- ✓ Run successfully in CI/CD environments without manual intervention
- ✓ Complete execution in under 5 seconds for the entire suite
- ✓ Produce clear, actionable failure messages when tests fail
- ✓ Be maintainable and easy to extend as server functionality grows
- ✓ Validate all user requirements: HTTP responses, status codes, headers, error handling, server lifecycle

**Tests Must NOT:**
- ✗ Modify production code beyond minimal testability exports
- ✗ Introduce flakiness or non-deterministic behavior
- ✗ Require external services or network connectivity
- ✗ Leave hanging processes or unclosed connections
- ✗ Create test pollution or side effects affecting other tests
